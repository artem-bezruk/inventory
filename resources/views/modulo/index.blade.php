...
@endsection
@section('breadcrumb-title', __('Modules'))
@section('breadcrumb')
				...
				<div class="card-header">
					...
					@if (session()->get('modulos')->modulos->c)
						<div class="text-right">
							<button type="button" class="btn btn-primary" onclick="crearModulo()"><i class="fas fa-plus"></i><span class="d-none d-md-inline">&nbsp;&nbsp;{{ __('Add') }}</span></button>
						</div>
					@endif
				</div>
				<div class="card-body">
...
@section('script')
	...
	<script type="text/javascript">
		...
		{
			var url = "{{ route('modulo.list', [ 'locale' => app()->getLocale() ]) }}";
			$.ajax({
				type: 'GET',
				url: url,
			...
			.done(function (response, statusText, jqXHR) {
				...
				if (jqXHR.status == 200) {
					...
					response.data.forEach( function(element, index) {
						...
						divBotonClose = '</div>';
						mostrar = '<button type="button" class="btn btn-info" data-toggle="tooltip" title="{{ __('Show') }}" onclick="mostrarModulo(' + "'" + element.urlMostrar + "'" +')"><i class="far fa-eye"></i></button>';
						editar = '<button type="button" class="btn btn-secondary" data-toggle="tooltip" title="{{ __('Edit') }}" onclick="editarModulo(' + "'" + element.urlEditar + "'" + ')"><i class="far fa-edit"></i></button>';
						eliminar = '<button type="button" class="btn btn-danger" data-toggle="tooltip" title="{{ __('Delete') }}" onclick="eliminarModulo(' + "'" + element.urlEliminar + "'" + ')"><i class="fas fa-trash-alt"></i></button>';
		...
		function crearModulo ()
		{
			$.ajax({
				type: 'GET',
				url: "{{ route('modulo.create', ['locale' => app()->getLocale()]) }}",

		...
		<div class="col-12">
			<div class="card">
				<div class="card-header">
					<div class="text-left">
						<h3 class="card-title">{{ __('Modules by Roles List') }}</h3>
					</div>
					@if (session()->get('modulos')->modulos_has_roles->c)
						<div class="text-right">
							<button type="button" class="btn btn-primary" onclick="crearModuloRol()"><i class="fas fa-plus"></i><span class="d-none d-md-inline">&nbsp;&nbsp;{{ __('Add') }}</span></button>
						</div>
					@endif
				</div>
				<div class="card-body">
...
@section('script')
	...
	<script type="text/javascript">
		...
		function listaModulosRoles ()
		{
			var url = "{{ route('modulorol.list', [ 'locale' => app()->getLocale() ]) }}";
			$.ajax({
				type: 'GET',
				url: url,
			...
			.done(function (response, statusText, jqXHR) {
				...
				if (jqXHR.status == 200) {
					...
					response.data.forEach( function(element, index) {
						...
						divBotonClose = '</div>';
						mostrar = '<button type="button" class="btn btn-info" data-toggle="tooltip" title="{{ __('Show') }}" onclick="mostrarModuloRol(' + "'" + element.urlMostrar + "'" +')"><i class="far fa-eye"></i></button>';
						editar = '<button type="button" class="btn btn-secondary" data-toggle="tooltip" title="{{ __('Edit') }}" onclick="editarModuloRol(' + "'" + element.urlEditar + "'" + ')"><i class="far fa-edit"></i></button>';
						eliminar = '<button type="button" class="btn btn-danger" data-toggle="tooltip" title="{{ __('Delete') }}" onclick="eliminarModuloRol(' + "'" + element.urlEliminar + "'" + ')"><i class="fas fa-trash-alt"></i></button>';
		...
		function crearModuloRol ()
		{
			$.ajax({
				type: 'GET',
				url: "{{ route('modulorol.create', ['locale' => app()->getLocale()]) }}",
		...
		function eliminarModuloRol (url)
		{
			Swal.fire({

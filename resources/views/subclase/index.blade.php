		...
		<div class="col-12">
			<div class="card">
				<div class="card-header">
					<div class="text-left">
						<h3 class="card-title">{{ __('Subclasses List') }}</h3>
					</div>
					@if (session()->get('modulos')->sub_clases->c)
						<div class="text-right">
							<button type="button" class="btn btn-primary" onclick="crearSubclase()"><i class="fas fa-plus"></i><span class="d-none d-md-inline">&nbsp;&nbsp;{{ __('Add') }}</span></button>
						</div>
					@endif
				</div>
				<div class="card-body">
...
@section('script')
	...
	<script type="text/javascript">
		...
		function listaSubclases ()
		{
			var url = "{{ route('subclase.list', [ 'locale' => app()->getLocale() ]) }}";
			$.ajax({
				type: 'GET',
				url: url,
			...
			.done(function (response, statusText, jqXHR) {
				...
				if (jqXHR.status == 200) {
					...
					response.data.forEach( function(element, index) {
						...
						divBotonClose = '</div>';
						mostrar = '<button type="button" class="btn btn-info" data-toggle="tooltip" title="{{ __('Show') }}" onclick="mostrarSubclase(' + "'" + element.urlMostrar + "'" +')"><i class="far fa-eye"></i></button>';
						editar = '<button type="button" class="btn btn-secondary" data-toggle="tooltip" title="{{ __('Edit') }}" onclick="editarSubclase(' + "'" + element.urlEditar + "'" + ')"><i class="far fa-edit"></i></button>';
						eliminar = '<button type="button" class="btn btn-danger" data-toggle="tooltip" title="{{ __('Delete') }}" onclick="eliminarSubclase(' + "'" + element.urlEliminar + "'" + ')"><i class="fas fa-trash-alt"></i></button>';
		...
		function crearSubclase ()
		{
			$.ajax({
				type: 'GET',
				url: "{{ route('subclase.create', ['locale' => app()->getLocale()]) }}",

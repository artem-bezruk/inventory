...
@section('script')
	...
	<script type="text/javascript">
		...
		function listaNomenclaturas ()
		{
			var url = "{{ route('nomenclatura.list', [ 'locale' => app()->getLocale() ]) }}";
			$.ajax({
				type: 'GET',
				url: url,
			...
			.done(function (response, statusText, jqXHR) {
				...
				if (jqXHR.status == 200) {
					...
					response.data.forEach( function(element, index) {
						...
						divBotonClose = '</div>';
						mostrar = '<button type="button" class="btn btn-info" data-toggle="tooltip" title="{{ __('Show') }}" onclick="mostrarNomenclatura(' + "'" + element.urlMostrar + "'" +')"><i class="far fa-eye"></i></button>';
						editar = '<button type="button" class="btn btn-secondary" data-toggle="tooltip" title="{{ __('Edit') }}" onclick="editarNomenclatura(' + "'" + element.urlEditar + "'" + ')"><i class="far fa-edit"></i></button>';
						eliminar = '<button type="button" class="btn btn-danger" data-toggle="tooltip" title="{{ __('Delete') }}" onclick="eliminarNomenclatura(' + "'" + element.urlEliminar + "'" + ')"><i class="fas fa-trash-alt"></i></button>';
		...
		function crearNomenclatura ()
		{
			$.ajax({
				type: 'GET',
				url: "{{ route('nomenclatura.create', ['locale' => app()->getLocale()]) }}",
		...
		function mostrarNomenclatura (url)
		{
			$.ajax({
				type: 'GET',
				url: url,
		...
		function editarNomenclatura (url)
		{
			$.ajax({
				type: 'GET',
				url: url,
		...
		function eliminarNomenclatura (url)
		{
			Swal.fire({
